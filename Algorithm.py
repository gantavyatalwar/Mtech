...
for f in files:
    with open(di+f) as news:
        print(news.name.split('/')[-1].rsplit('.')[-2].split('-')[-1].strip(), end='')
        l=news.readlines()
        k=v=0
        p,n,c=0,0,0
        words=0
        for i in l:
            k,v,r=sentence_score(i)
            c=c+r
            p=p+k
            n=n+v
            #if k>v:
        ...
        print(' ',round(p,2),round(n,2),c,round(final1,2),round(final2,2))
        positive.append(final1)
        negative.append(-final2)
print(positive)
print(negative)
